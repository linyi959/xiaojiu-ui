...
import type { Message, ContentBlock, Phase } from "@/stores/hermes/chat";
...
import ToolMiniCard from "./ToolMiniCard.vue";
...
// 优先使用 phases 的原始顺序渲染实时/历史阶段链。
// 旧逻辑会把所有 thinking 合成一个块、所有 tool_call 统一挂底，导致实时顺序变成：整段思考 → 全部工具。
const orderedPhases = computed(() => props.message.phases || []);
const hasPhaseTimeline = computed(() =>
  props.message.role === 'assistant' && orderedPhases.value.length > 0,
);

const fallbackToolCallPhases = computed(() =>
  hasPhaseTimeline.value ? [] : (props.message.phases || []).filter((p) => p.type === 'tool_call')
);
...
/** Groups consecutive same-type phases while preserving interleaving between
 *  different types. E.g. [thinking, tool, thinking, tool, final] stays as
 *  four groups rather than collapsing into [all thinking, all tools, final]. */
const timelineGroups = computed<TimelineGroup[]>(() => {
  const phases = orderedPhases.value
  const groups: TimelineGroup[] = []
  for (const phase of phases) {
    if (phase.type === 'thinking' && !phase.content) continue
    if (phase.type === 'final' && !phase.content) continue
    const last = groups[groups.length - 1]
    if (last && last.type === phase.type) {
      last.phases.push(phase)
    } else {
      groups.push({ type: phase.type as TimelineGroup['type'], phases: [phase] })
    }
  }
  return groups
})
...
const hasThinking = computed(() =>
  hasPhaseTimeline.value || hasReasoningField.value || parsedThinking.value.hasThinking,
);
      ...
      <div class="msg-body">
        ...
        <div class="msg-content" :class="message.role">
          <div class="message-bubble" :class="{ system: isSystem, 'speech-playing': isPlayingThisMessage && !isPausedThisMessage }">
            ...
            <div
              v-if="hasThinking && !hasPhaseTimeline"
            ...
            >
              ...
              <div v-if="fallbackToolCallPhases.length > 0" class="thinking-tool-chain">
                <ToolMiniCard
                  v-for="phase in fallbackToolCallPhases"
                  :key="phase.id"
                  :message="{
                    toolName: phase.toolName,
                    toolPreview: phase.toolPreview,
                    toolArgs: phase.toolArgs,
                    toolResult: phase.toolResult || phase.content,
                    toolStatus: phase.toolStatus,
                    toolDuration: phase.toolDuration,
                    content: phase.content || phase.toolResult || '',
                  }"
                  embedded
                />
              </div>
            ...
            <div v-if="hasPhaseTimeline" class="phase-timeline">
              <template v-for="group in timelineGroups" :key="group.phases[0].id">
                <template v-if="group.type === 'thinking'">
                  <div
                    v-for="phase in group.phases"
                    :key="phase.id"
                    class="thinking-block phase-thinking-block"
                    :class="{ expanded: thinkingExpanded }"
                  >
                    <div class="thinking-header" @click="toggleThinking">
                      <svg
                      ...
                      <span class="thinking-icon">💭</span>
                      <span class="thinking-label">
                        {{ phase.isStreaming ? t('chat.thinkingInProgress') : t('chat.thinkingLabel') }}
                      </span>
                      <span class="thinking-meta">· {{ t('chat.thinkingChars', { count: phase.content.length }) }}</span>
                    </div>
                    <div v-if="thinkingExpanded" class="thinking-body">
                ...
                <div v-else-if="group.type === 'tool_call'" class="thinking-tool-chain">
                  <ToolMiniCard
                    v-for="phase in group.phases"
                    :key="phase.id"
                    :message="{
                ...
                <div v-else-if="group.type === 'final'">
                  <div v-for="phase in group.phases" :key="phase.id" class="phase-final">
                    <MarkdownRenderer :content="phase.content" />
                  </div>
                </div>
              </template>
            ...
            <MarkdownRenderer
              v-if="!hasPhaseTimeline && parsedThinking.body && message.role === 'assistant'"
            ...
            <MarkdownRenderer
              v-if="message.role === 'assistant' && message.content && !parsedThinking.body && !hasPhaseTimeline"
...
.thinking-tool-chain {
  ...
  &::before {
    ...
    background: linear-gradient(
      ...
      rgba(var(--text-muted-rgb), 0.18)
    );
  }

  :deep(.tool-mini-card.embedded) {
    align-self: flex-start;
    width: min(640px, 100%);
    margin: 0;
  }

  :deep(.tool-mini-card.embedded)::after {
    content: "";
    position: absolute;
    left: -12px;
    top: 22px;
...
.thinking-block .thinking-tool-chain {
  margin-top: 8px;
  margin-left: 4px;
}
